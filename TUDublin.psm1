...
<#
.Synopsis
    Retrieves security logs from a client PC
.DESCRIPTION
    Long description
.EXAMPLE
...
#>
    ...
    Process {
        ForEach ($Computer in $ComputerName) {
            $Logons = Get-WinEvent -FilterHashtable @{logname='security';id=4624;data=$UserName} -ComputerName $Computer -Credential $Cred -ErrorAction SilentlyContinue |
                
                Where-Object {((2,3,7,10,11) -contains $_.Properties[8].Value) -and $_.TimeCreated -gt $Date }  |

                Select-Object -Property TimeCreated,
                    @{label='UserName';expression={$_.properties[5].value}},
                    @{label='LogonType';expression={$_.properties[8].value}},
                    @{label='LogonID (Dec)';expression={$_.properties[7].value}},
                    @{label='Linked LogonID (Dec)';expression={$_.properties[25].value}},
            ...
            $AccountLogoff = Get-WinEvent -FilterHashtable @{logname='security';id=4634;data=$UserName} -ComputerName $Computer -Credential $Cred -ErrorAction SilentlyContinue|
                ...
                Select-Object -Property TimeCreated,
                    ...
                    @{label='LogonID (Dec)';expression={$_.properties[3].value}},
                    @{label='Domain';expression={$_.properties[2].value}},
                    @{label='Event';expression={"AccountLogoff"}}


            $UserInitiatedLogoff = Get-WinEvent -FilterHashtable @{logname='security';id=4647;data=$UserName} -ComputerName $Computer -Credential $Cred -ErrorAction SilentlyContinue |
    ...
    End {
            $Logons, $AccountLogoff, $UserInitiatedLogoff
    }
